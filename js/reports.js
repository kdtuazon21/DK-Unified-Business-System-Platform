...
const Reports = {
    ...
    renderPnL() {
        ...
        const summaries = active.map(co => {
            ...
            const expenses = DataStore.expenses.filter(e => e.company === co).reduce((s, e) => s + e.amount, 0);
            const netIncome = revenue - expenses;
            return { co, name: DataStore.companies[co]?.name || co, revenue, expenses, netIncome, ...summary };
        });
            ...
            <div class="card-body no-padding">
                <table class="data-table">
                    <tbody>
                        ...
                        <tr><td colspan="2" style="padding-left:24px">Service Revenue</td><td style="text-align:right">${Utils.formatCurrency(c.revenue * 0.85)}</td></tr>
                        <tr><td colspan="2" style="padding-left:24px">Other Income</td><td style="text-align:right">${Utils.formatCurrency(c.revenue * 0.15)}</td></tr>
                        ...
                        <tr><td colspan="2" style="padding-left:24px">Operating Expenses</td><td style="text-align:right">(${Utils.formatCurrency(c.expenses * 0.6)})</td></tr>
                        <tr><td colspan="2" style="padding-left:24px">Payroll</td><td style="text-align:right">(${Utils.formatCurrency(c.expenses * 0.3)})</td></tr>
                        <tr><td colspan="2" style="padding-left:24px">Utilities & Other</td><td style="text-align:right">(${Utils.formatCurrency(c.expenses * 0.1)})</td></tr>
    ...
    renderCashFlow() {
        ...
        const monthlyOutflow = monthlyInflow.map(v => v * (0.55 + Math.random() * 0.15));
        const netFlow = monthlyInflow.map((v, i) => v - monthlyOutflow[i]);
        let runningBalance = 1500000;
    ...
    exportReport() {
        App.showToast('Report exported (CSV/PDF generation would integrate here)', 'info');
    }
};
